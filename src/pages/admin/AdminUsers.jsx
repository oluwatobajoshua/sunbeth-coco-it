...
const UserForm = ({ initial, stations, onSave, onCancel }) => {
  const [form, setForm] = React.useState(initial || { name:'', email:'', role:'station_manager', stationId:'', active:true });
  return (
    <div className="card" style={{ padding: 12 }}>
      <div className="d-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 8 }}>
        ...
        <select className="btn-outline" value={form.role} onChange={e=>setForm({ ...form, role: e.target.value })}>
...
const AdminUsers = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [u, s] = await Promise.all([listUsersAdmin(), listStations()]);
      setList(u); setStations(s);
    } finally { setLoading(false); }
  };
  React.useEffect(()=>{ load(); },[]);

  const onCreate = async (data) => { await createUserAdmin(data); setCreating(false); load(); };
  const onUpdate = async (id, data) => { await updateUserAdmin(id, data); setEditing(null); load(); };
  ...
  return (
    <section>
      ...
      {creating && canManage && <UserForm stations={stations} onSave={onCreate} onCancel={()=>setCreating(false)} />}
      {loading ? (
        ...
        <div className="table" role="table" aria-label="Users">
          ...
          {list.map(item => (
            <div key={item.id} className="table-row" role="row">
              ...
              <div className="cell" role="cell">{item.role}</div>
      ...
      {editing && canManage && <UserForm stations={stations} initial={editing} onSave={(data)=>onUpdate(editing.id, data)} onCancel={()=>setEditing(null)} />}
    </section>
  );
};
