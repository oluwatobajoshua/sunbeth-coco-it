...
import { listStations, createStation, updateStation, deleteStation } from '../../services/adminService';
import { useAuth } from '../../hooks/useAuth';
import { fetchPermissionsMatrix, hasPermission, normalizeRole } from '../../utils/permissions';

const StationForm = ({ initial, onSave, onCancel }) => {
  const [form, setForm] = React.useState(initial || { code: '', name: '', city: '', region: '', active: true });
  return (
    <div className="card" style={{ padding: 12 }}>
      <div className="d-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 8 }}>
        <input className="input" placeholder="Code (stationId)" value={form.code} onChange={e=>setForm({ ...form, code: e.target.value })} />
...
const AdminStations = () => {
  ...
  const canManage = matrix ? hasPermission(matrix, roleKey, 'manage_stations') : false;

  const load = async () => {
    setLoading(true);
    try { setList(await listStations()); } finally { setLoading(false); }
  };
  React.useEffect(()=>{ load(); },[]);

  const onCreate = async (data) => { await createStation(data); setCreating(false); load(); };
  const onUpdate = async (id, data) => { await updateStation(id, data); setEditing(null); load(); };
  const onDelete = async (id) => { if (window.confirm('Delete station?')) { await deleteStation(id); load(); } };
  ...
  return (
    <section>
      <div className="section-header" style={{ display: 'flex', justifyContent: 'space-between' }}>
        <h3 className="small" style={{ marginBottom: 0 }}>Stations</h3>
        <button className="btn btn-accent" onClick={()=>setCreating(true)} disabled={!canManage}><i className="fas fa-plus"></i> New Station</button>
      </div>
      {creating && canManage && <StationForm onSave={onCreate} onCancel={()=>setCreating(false)} />}
      {loading ? (
